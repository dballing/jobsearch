...
./preview_resume.sh <file>   # print a resume (.md/.txt/.pdf/.docx) EXACTLY as the scorer will see it — text to stdout, a format/size/truncation summary to stderr. Use to catch PDF/DOCX extraction garble before configuring resume_file.
...
| `viability.py` | Shared scoring helpers: `prompt_hash`, `score_job`, `compose_candidate_prompt` (folds a configured resume into the candidate prompt, mandating the `application_competitiveness` factor). |
| `resume.py` | Extracts a `[viability].resume_file` (`.md`/`.txt`/`.pdf`/`.docx`) to plain text for the scorer; `pypdf`/`python-docx` are lazy imports. `config.load_config` folds the text into the viability prompt (so it's covered by the staleness hash) and registers the file for mtime reload; failures are loud (`ConfigError`), not fail-soft. Letter-spaced PDF headers (`P R O F E S S I O N A L`) are auto-repaired (`_collapse_letterspaced`, conservative/line-wise). Also a CLI (`preview_resume.sh`) that prints the extracted text so remaining PDF/DOCX garble is caught before use. |
...
- **Per-lens state lives in `job_search_state(job_id, search_id, …)`** — status, viability (+reason/factors/hash/needs_rescored), the `salary_min_actual`/`salary_max_actual`/`salary_currency_actual`/`geo_fit_actual` overrides, `applied_at`, and per-lens `history`. A row's existence IS the job's membership in that search. Most override columns have a **dormant** twin on `jobs` (kept for rollback, migrated once to `__default__`); nothing reads them (a poison test guards this). `salary_currency_actual` is the exception — it's a newer field with **no** `jobs` counterpart (a plain nullable column, NULL ⇒ use the feed's shared `jobs.salary_currency`), so don't look for a dormant `jobs.salary_currency_actual`. Shared, objective posting facts stay on `jobs`: notes, attachments, `salary_currency` (the feed currency), `description_actual`, `company_actual`, `title_actual`, `work_arrangement_actual`.
...
- **Live config reload — read config through `current_config()`.** `app.py` does **not** hold a frozen config global anymore; `current_config()` reloads `config.toml` (+ per-search files) on any mtime change, fail-soft (a malformed/half-saved file keeps the last-good config and retries), and snapshots per request via `flask.g`. So display/scoring config (search names, labels, viability prompts, feeds, aliases, AI settings) takes effect on the next request with **no restart**. Any new config read in `app.py` must go through `current_config()` (or the `_label_names()`/`_valid_search_ids()`/`_lens_colors()`/`_viability_hashes()` helpers that wrap it) — never cache config in a module global, or your read goes stale. **The deliberate exceptions are `_PINNED_CONFIG_FIELDS` (`db_path`, `uploads_dir`)**: captured once at startup (they bind the open DB/WAL/writer-lock and the attachments dir), so editing one needs a restart — `config_drift()` drives a sticky, app-wide banner saying so (env-sourced values, e.g. `JOBSEARCH_DB`, never "drift" since the file value is inert). Keep that pinned list tiny; a field *not* on it is live by default (the safe direction). Batch entry points (`ingest.py`, `rescore_viability.py`) are separate short-lived processes that load config fresh per run, so none of this applies to them.

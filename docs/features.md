...
**Currency.** Salaries are shown in the currency the feed reported (`€`, `£`, `$`, …); a currency without a dedicated glyph shows as its code (e.g. `CHF 120k`), and a posting with no currency defaults to `$`. The same currency also reaches the viability scorer, so a €/£ role is comp-judged in its real currency rather than mislabeled as dollars. Next to the salary-override fields is a **currency selector** for correcting a feed that stamped the wrong currency (e.g. a euro band tagged USD). It's independent of the min/max band — you can change just the currency and leave the numbers as-is; leaving it on "feed default" keeps whatever the feed reported. The stored/displayed figure is never rewritten into dollars: `€100k` stays `€100k`. Like the band, the currency override is per-lens, fans out across the matched group, records in History, and flags a re-score.

**USD equivalent on hover.** A non-USD salary (table rows, grouped rows, and the preview panel) carries a dotted underline; hovering shows an approximate USD figure, e.g. `≈ $130k – $160k USD`. This is display-only — the underlying value stays in its real currency and the scorer still judges it in that currency. Rates come from a free public FX endpoint, cached on disk for 24h (fetched at most once a day) and fail-soft: if rates can't be fetched and nothing is cached, the salary simply shows with no tooltip. The `≈` is a reminder that it's a daily-rate approximation, not a booked conversion, and a currency with no available rate gets no tooltip rather than a wrong number.
...
- **Per applied**: total spend ÷ the number of **tracked** jobs in the lens whose status is in the applied family (applied, interviewing, offered, rejected, ghosted, withdrawn). "Tracked" means the job has ledger spend in that lens *and* you applied to it on or after the day tracking began. Both halves matter: a job you applied to months ago picks up a ledger row the moment it's rescored, and counting it would divide today's spend by an application whose own cost was never recorded — deflating the figure just as a backfill would.
...
- **Optional resume (`[viability].resume_file`).** Point it at a resume file (`.md`/`.txt`, `.pdf`, or `.docx`) and its text is folded into your candidate profile as **authoritative, ground-truth** evidence of your **actual** experience — so `role requirements fit` is judged against your real background, not just how you described yourself. It also **mandates** an extra factor, `application competitiveness`: the **employer's-eye** view of how likely you are to get a positive response (a recruiter screen / callback) versus being filtered out — a *distinct* question from whether the role is a good fit **for you** — with its own signed −2…+2 contribution in the breakdown, so you can see how much "would they even respond?" moved the rating. The resume is per-search (each lens can use one tailored to that kind of role) and its **text** is part of the staleness hash, so editing the resume re-scores; `.pdf`/`.docx` need `pypdf`/`python-docx` respectively (only if you use them). Because PDF/DOCX extraction can garble multi-column layouts or tables, run **`./preview_resume.sh <file>`** first to see the exact text the scorer will use. See [Configuration → Viability scoring](configuration.md#viability-scoring-viability).
- A SHA-256 hash of the prompt is stored with each score. On subsequent runs, only jobs with a missing or stale hash are re-scored. (When a resume is configured, its extracted text is part of that prompt, so editing the resume marks scores stale exactly as editing the prompt does.)
